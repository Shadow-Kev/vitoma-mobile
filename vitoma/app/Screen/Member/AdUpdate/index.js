...
import Styles from '@Screen/Member/AdUpdate/Style';
...
import { Image, StatusBar, TextInput, View, ActivityIndicator } from 'react-native';
import AdUpdateState from './State';
...
class AdUpdate extends React.Component {
    constructor(props) {
      ...
      // loading when fetching the ad detail
      this.loading = true

      // Loader when sending the update request
      this.showLoader = false;
    }

    componentWillMount() {
      const { state } = this.props.navigation
      const postDetailConfig = state.params.postDetailConfig

      this.postDetailConfig = postDetailConfig

      this.fetchPostDetail(this.postDetailConfig.postId)
    }

    fetchPostDetail = (postId) => {
      this.loading = true

      let url = `${Config.URL.entryPoint}/ads/${postId}/details`

      fetch(url).then(
        response => response.json(),
        error => {
          Logger.log({ ...error, url, file: "Screen/Member/AdUpdate/index.js" })
        }
      )
      .then(data => {
        if(data) {
          this.loading = false
          this.userId = data.owner.userId
          this.setState(AdUpdateState(data));
        }
      })
    }
    ...
    onPublished = () => {
      ...
      if(!invalidFields.length) {
        ...
        var url = Config.URL.entryPoint + '/ads/' + this.postDetailConfig.postId + '/update';
        ...
        .then((data) => {
          ...
          if(data && data.status == 'SUCCESS') {
            ...
            NavigationService.navigate('MemberAdPublished', { postDetailConfig: this.postDetailConfig })
          } else {
            Events.toast(Languages.Validations.notUpdatedAd)
        ...
        return <Container style={Style.bgMain}>
            <Header style={Style.navigation}>
                ...
                <View style={Style.actionBarMiddle}>
                    <Text style={Style.actionBarText}>{Languages.AdUpdate.updateAd.toUpperCase()}</Text>
                </View>
                ...
                <View style={Styles.section}>
...

                    {
                      ...
                      this.displayTextInput({
                        ...
                        placeholder: (this.state.emailPhoneConfig.value == 'showBoth' ||
                                      ...
                                      Languages.AdCreate.required : Languages.AdCreate.optional,                        value: this.state.email.value,
                        onChange: this.onChangeText,
                        validateField: this.validateField
                      })
                    }
                    ...
                    <View style={Styles.itemFooter}>
                        <Button iconLeft transparent style={Styles.itemBtnActive} onPress={this.onPublished} disabled={this.showLoader}>
                            ...
                            <Text style={Styles.itemText}>{Languages.AdUpdate.publish}</Text>
                        </Button>
                    </View>
...
}, { changeDefaultPhoneCountryCode })(AdUpdate)
